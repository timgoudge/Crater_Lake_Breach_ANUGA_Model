...
import math
import numpy as np
from model_params import Wd, Sd, porosity, grainD, frictionscheme, gravity, dthresh, maxrate, bed_tau_crit, m, Ke, bedcons
        ...
        if self.indices is not []:     # empty list no polygon - return
            ...
            if len(ind)>0:
                ...
                if (numoferoding>0) and (numofdeposit>0):            #stop if there isn't both erosion and deposition.
                    ...
                    if np.max(np.abs(dz))>maxdz:                       #rate limiter.  Maxrate is the most that a cell can change in a timestep.  This fights potential stability problems.
